...
auto rotate(ATp angle) {
    angle = radians(angle);
    auto res = Self::unit();
    res[1, 1] = res[0, 0] = xcmath::cos(angle);
    res[0, 1] = -(res[1, 0] = xcmath::sin(angle));
    return res * const_self;
}
...
constexpr auto translate(const VTp& v) {
    ...
    res[0, size_ - 1] += v[0];
    res[1, size_ - 1] += v[1];
    if constexpr (size_ == 4) res[2, size_ - 1] += v[2];
    return res;
}
IMPL_METHOD_END();
...
constexpr auto scale(const VTp& v) {
    ...
    res[0, 0] *= v[0];
    res[1, 1] *= v[1];
    if constexpr (size_ == 4) res[2, 2] *= v[2];
    return res;
}
template <class VTp>
...
constexpr auto scale(VTp s) {
    ...
    res[0, 0] *= s;
    res[1, 1] *= s;
    if constexpr (size_ == 4) res[2, 2] *= s;
    return res;
}
IMPL_METHOD_END();
